...
def e2(z: torch.Tensor) -> torch.Tensor:
    return 0.5 * torch.square((z[1] - math.sin(2 * math.pi * Z[0] / 4)) / 0.4)
...
def e3(z: torch.Tensor) -> torch.Tensor:
    return -torch.log(
        torch.exp(- 0.5 * torch.square((z[1] - math.sin(2 * math.pi * z[0] / 4)) / 0.35)) +
        torch.exp(- 0.5 * torch.square((z[1] - math.sin(2 * math.pi * z[0] / 4)
